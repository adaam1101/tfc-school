...
/* GET /api/ratings/summary — public */
...
/* GET /api/ratings/recent — public, latest 12 */
ratingRouter.get("/recent", async (_req, res, next) => {
  try {
    const recent = await Rating.find({ stars: { $gte: 1 } })
      .sort({ createdAt: -1 })
      .limit(12)
...
/* POST /api/ratings — authenticated, upsert one per user */
...
/* GET /api/ratings/mine — authenticated */
