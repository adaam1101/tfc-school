...
export default function AppLayout({ title, subtitle, children }) {
  ...
  const roleGradients = { admin: "from-violet-600 to-purple-700", teacher: "from-sky-600 to-blue-700", student: "from-sky-600 to-emerald-700" };
    ...
    <div className="flex min-h-screen flex-col bg-gradient-to-br from-slate-50 via-slate-100 to-slate-50 text-slate-950">
      <header className="sticky top-0 z-40 border-b border-slate-200/80 bg-white/85 backdrop-blur-md shadow-sm">
        <div className="mx-auto flex max-w-7xl items-center justify-between gap-4 px-4 py-3 sm:px-6 lg:px-8">
          <Link to={`/${user?.role || "admin"}`} className="group flex items-center gap-3">
            <img src={schoolLogo} alt="TFC School" className="h-10 w-10 rounded-xl object-contain ring-2 ring-slate-100 transition group-hover:ring-sky-200" />
          ...
          <div className="flex items-center gap-2">
            {(user?.role === "admin" || user?.role === "teacher") && (
              <Link to="/rfid-attendance" className={`inline-flex items-center gap-2 rounded-xl px-4 py-2 text-sm font-bold transition-all duration-200 ${isRfid ? "bg-gradient-to-r from-sky-600 to-emerald-700 text-white shadow-sm" : "border border-slate-200 bg-white text-slate-700 hover:border-sky-200 hover:bg-sky-50 hover:text-sky-800"}`}>
                <ScanLine className="h-4 w-4" /><span className="hidden sm:inline">RFID</span>
              </Link>
            )}
            <button onClick={() => setShowRating(true)} className="inline-flex items-center gap-2 rounded-xl border border-amber-200 bg-amber-50 px-3 py-2 text-sm font-bold text-amber-700 transition-all hover:bg-amber-100">
            ...
            <button type="button" onClick={logout} className="inline-flex items-center gap-2 rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm font-medium text-slate-600 shadow-sm transition-all hover:border-rose-200 hover:bg-rose-50 hover:text-rose-700">
      ...
      <main className="mx-auto w-full max-w-7xl flex-1 px-4 py-8 sm:px-6 lg:px-8">
        <div className="mb-8 animate-fade-slide-up">
          <h1 className="text-2xl font-black tracking-tight text-sky-800 sm:text-3xl">{title}</h1>
