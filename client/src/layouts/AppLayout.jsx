...
export default function AppLayout({ title, subtitle, children }) {
  ...
  const roleGradients = { admin: "from-violet-600 to-purple-700", teacher: "from-sky-600 to-blue-700", student: "from-teal-600 to-emerald-700" };
      ...
      <header className="sticky top-0 z-40 border-b border-slate-200/80 bg-white/85 backdrop-blur-md shadow-sm">
        <div className="mx-auto flex max-w-7xl items-center justify-between gap-4 px-4 py-3 sm:px-6 lg:px-8">
          <Link to={`/${user?.role || "admin"}`} className="group flex items-center gap-3">
            <img src={schoolLogo} alt="TFC School" className="h-10 w-10 rounded-xl object-contain ring-2 ring-slate-100 transition group-hover:ring-teal-200" />
